...
(function (exports, Laya) {
    ...
    class WxBannerGroup {
        ...
        onAppShow() {
            ...
            Laya.stage.off(zs.laya.platform.PlatformMgr.APP_SHOW, this, this.onAppShow);
            this.show();
        ...
        onAppHide() {
            ...
            Laya.stage.off(zs.laya.platform.PlatformMgr.APP_HIDE, this, this.onAppHide);
            Laya.stage.on(zs.laya.platform.PlatformMgr.APP_SHOW, this, this.onAppShow);
        ...
        lockHideExposure() {
            for (var i = 0; i < this.showExposureIds.length; i++) {
        ...
        show() {
            ...
            Laya.stage.on(zs.laya.platform.PlatformMgr.APP_HIDE, this, this.onAppHide);
            wxBanner.show();
        }
        ...
        showExposure(showType) {
            ...
            //从剩余的未显示的Banner 曝光显示
            if (ids.length > 0) {
                var showExposureId = ids[zs.laya.platform.MathUtils.random(0, ids.length)];
    ...
    class WxBannerMgr {
        ...
        initBannerGroupBySign(sign, length, autoChange = false, isReset = false) {
            ...
            if (this.bannerGroupArray[sign]) {
                ...
                var wxBanner = this.bannerGroupArray[sign].getCurrentShowBanner();
                if (!wxBanner) return;
                if (wxBanner.showTimes >= this.bannerGroupArray[sign].showCountRefresh) {
                    wxBanner.destroy();
                    wxBanner.createBanner();
                    console.log("次数到了，刷新");
                }
            ...
            length = this.adUnitIdData.length <= 1 ? this.adUnitIdData.length : length;

            if (isReset || this.bannerIds.length < length) {
                this.bannerIds = [];
                for (var i = 0; i < this.adUnitIdData.length; i++) {
                    this.bannerIds[i] = i;
                }
            }
            ...
            for (var i = 0; i < length; i++) {
                var bannerId = this.bannerIds.splice(zs.laya.platform.MathUtils.random(0, this.bannerIds.length), 1)[0];
        ...
        lockHide() {
            for (var i = this.bannerGroupArray.length - 1; i >= 0; i--) {
                var bannerGroup = this.bannerGroupArray[i];
